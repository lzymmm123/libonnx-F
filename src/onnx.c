		...
		case ONNX__TYPE_PROTO__VALUE_TENSOR_TYPE:
			{
				...
				ndim = -2;
				// printf("v name %s\n",v->name);
				// if(strcmp(v->name, "TFNodes/yolo_evaluation_layer_1/mul_6:0")==0){
				if(strcmp(v->name, "TFNodes/yolo_evaluation_layer_1/mul_6:0")==0){
					printf("=================\n");
					printf("has shape %d\n",v->type->tensor_type->shape!=NULL);
				}
						...
						for(i = 0; i<ndim; ++i){
							switch (v->type->tensor_type->shape->dim[i]->value_case) {
								...
								case ONNX__TENSOR_SHAPE_PROTO__DIMENSION__VALUE_DIM_PARAM:
									if(strcmp(v->type->tensor_type->shape->dim[i]->dim_param,"N")==0){
										dims[i] = 1;
									}else{
										dims[i] = 1;
									}
...
#define translate(type,bits) \
{ \
	type *p = (type *)t->datas; \
	type *q = (type *)o->raw_data.data; \
	if(t->ndata>0) {\
		n = min(t->ndata,(size_t)o->raw_data.len / sz); \
		for(i=0; i<n; ++i) { \
			p[i] = le##bits##_to_cpu(q[i]); \
		}\
	}\
}


#define translate8(type) \
{ \
	type *p = (type *)t->datas; \
	type *q = (type *)o->raw_data.data; \
	if(t->ndata>0) {\
...
#define translatemin(type,m) \
{\
	n = min(t->ndata,(size_t)o-> n_##type##_data); \
	if((n>0) && t->datas && o->type##_data){\
	  memcpy(t->datas,o->type##_data, m*n); \
	}\
}
				...
				if((o->raw_data.len>0) && o->raw_data.data){
					switch (o->data_type) {
						...
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT16:
							translate(uint16_t,16);
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__INT16:
							translate(int16_t,16);
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__INT32:
							translate(int32_t,32);
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__INT64:
							translate(int64_t,64);
							break;
						...
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT32:
							translate(uint32_t,32);
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT64:
							translate(uint64_t,64);
							break;
					...
					switch (o->data_type) {
						case ONNX__TENSOR_PROTO__DATA_TYPE__FLOAT:
							translatemin(float,sizeof(float));
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT8:
						case ONNX__TENSOR_PROTO__DATA_TYPE__INT8:
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT16:
						...
						case ONNX__TENSOR_PROTO__DATA_TYPE__FLOAT16:
						case ONNX__TENSOR_PROTO__DATA_TYPE__BFLOAT16:
							translatemin(int32,sz);
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__STRING:
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__INT64:
							translatemin(int64,sizeof(int64_t));
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__DOUBLE:
							translatemin(double,sizeof(double));
							break;
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT32:
						case ONNX__TENSOR_PROTO__DATA_TYPE__UINT64:
							translatemin(uint64,sz);
						case ONNX__TENSOR_PROTO__DATA_TYPE__COMPLEX64:
						case ONNX__TENSOR_PROTO__DATA_TYPE__COMPLEX128:
							break;
						default:
...
struct onnx_graph_t * onnx_graph_alloc(struct onnx_context_t * ctx, Onnx__GraphProto * graph){
	...
	for(i = 0; i < graph->n_output; i++){
		v = graph->output[i];
		printf("graph output- %s\n",v->name);
		if(!onnx_tensor_search(ctx, v->name)){
			t = onnx_tensor_alloc_from_value_info(v);
			if(t) hmap_add(ctx->map, t->name, t);
		}
	}
	...
	for(i = 0; i < graph->n_node; i++)
	{
		for(j = 0; j < graph->node[i]->n_input; j++)
		{
			...
			if(strcmp(name, "TFNodes/yolo_evaluation_layer_1/mul_6:0")==0){
				printf("node input name - %s node name %s\n",name, graph->node[i]->name);
				printf("search %d\n",!onnx_tensor_search(ctx, name));
				// for(k = 0; k < graph->n_initializer; k++){
				// 	printf("ini name %s\n",graph->initializer[k]->name);
				// }
				// printf("this %s\n",o->name);
			}
			if(!onnx_tensor_search(ctx, name))
			{
				for(k = 0; k < graph->n_initializer; k++)
				{
					if(strcmp(graph->initializer[k]->name, name) == 0)
					{
						...
						// if(strcmp(o->name, "TFNodes/yolo_evaluation_layer_1/Reshape_11__95:0")==0){
						// if(strcmp(o->name, "TFNodes/yolo_evaluation_layer_1/mul_6:0")==0){
						if(strcmp(o->name, "resnetv15_stage1_batchnorm2_fwd")==0){
							printf("------this %s\n",o->name);
							printf("------o ndims %ld dims0 %d\n",o->n_dims,(int)o->raw_data.len);
						}
						if(o)
						{
							int ndim = o->n_dims;
	...
	for(i = 0; i < g->nlen; i++)
	{
		...
		printf("init -------------- %ld\n",strlen(n->proto->name));
		printf("init %s %d\n",n->proto->name,n->init==NULL);
		// printf("== init 1 %s %d\n",n->proto->name,n->init(n));
		if(n->init)
		{
			printf("basdxasjlkdjalsk\n");
			printf("init mid %s %d\n",n->proto->name,n->init(n));
			if(n->init(n) <= 0)
			{
				printf("init after %d\n",n->init(n));
				...
				printf("free op name %s\n",n->proto->op_type);
				free(g);
				return NULL;
			}
			printf("init mid1 %s %d\n",n->proto->name,n->init(n));
			printf("init mids %s %d\n",n->proto->name,n->init(n));
		}
		// printf("init mid2 %s %d\n",n->proto->name,n->reshape(n));
		// if(n->reshape)
		// 	n->reshape(n);
		// printf("init mid3 %s %d\n",n->proto->name,n->reshape(n));
	}
